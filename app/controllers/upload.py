    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            try:
                ...
                # Initialiser le fichier ZIP
                zip_buffer = BytesIO()
                with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED, False) as zip_file:
                    # Filtrer les données par Manufacturer
                    grouped = df.groupby('Manufacturer')

                    for manufacturer, group_df in grouped:
                        # Créer des DataFrames filtrés
                        df_logic_duplicate = group_df[group_df['ErrorType'].str.startswith('Logical Duplicate with')]
                        df_perfect_duplicate = group_df[group_df['ErrorType'].str.startswith('Perfect Duplicate with')]
                        df_missing_relationship = group_df[group_df['ErrorType'].str.startswith('Missing relationship')]
                        ...
                        # Classifier et traduire les types d'erreurs
                        ...
                        # Ajouter les fichiers dans le dossier du Manufacturer
                        folder_name = f"{manufacturer}/"
                        zip_file.writestr(f"{folder_name}Logic_duplicate_{file.filename}_{today}.csv", df_logic_duplicate.to_csv(index=False, encoding="utf-8", sep=";"))
                        zip_file.writestr(f"{folder_name}Perfect_duplicate_{file.filename}_{today}.csv", df_perfect_duplicate.to_csv(index=False, encoding="utf-8", sep=";"))
                        zip_file.writestr(f"{folder_name}Missing_relationship_{file.filename}_{today}.csv", df_missing_relationship.to_csv(index=False, encoding="utf-8", sep=";"))
                        zip_file.writestr(f"{folder_name}ALL_Errors_report_{file.filename}_{today}.xlsx", excel_buffer.getvalue())

                    # Ajouter le fichier KPI global (pas spécifique à un Manufacturer)
